...
复杂度不是程序运行的精确秒数，而是观察输入规模变大时，程序的工作量或额外内存如何增长。

这里的 `n` 表示输入数组中元素的数量：

- 访问一个固定位置，例如 `nums[0]`，工作量基本不随 `n` 变化，可以记作 `O(1)`；
- 一个循环遍历 `n` 个元素，工作量大约随 `n` 增长，可以记作 `O(n)`；
- 两层都遍历 `n` 个元素，比较次数大约是 `n × n`，可以记作 `O(n²)`。

暴力版 Two Sum 使用两层循环，因此时间复杂度是 `O(n²)`。

`unordered_map` 版本只遍历一次数组。每次查找和插入平均可以看作 `O(1)`，所以总时间复杂度是平均 `O(n)`。map 最多保存 `n` 个数字，因此额外空间复杂度是 `O(n)`。

`unordered_map` 的 `O(1)` 是平均情况；发生大量哈希冲突时，单次操作的最坏情况可能退化。当前阶段先掌握平均复杂度即可。

## 下一步

在不改变题目功能的前提下，进一步熟悉：

1. 用更多简单例子巩固 `O(1)`、`O(n)` 和 `O(n²)`；
2. `const std::vector<int>&` 的含义；
3. 使用迭代器版本重构当前提交。